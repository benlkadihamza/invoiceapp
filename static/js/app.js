...
function initUIUtilities() {
    ...
    // Global listener for PDF share/download buttons
    document.addEventListener('click', async (e) => {
        const btn = e.target.closest('.btn-pdf-action');
        if (!btn) return;
        e.preventDefault();
        const invoiceId = btn.dataset.invoiceId;
        const clientName = btn.dataset.clientName || 'facture';
        ...
        try {
            await shareOrDownloadPdfById(invoiceId, clientName, invoiceNum);
        } catch (err) {
            console.error(err);
...
function showPdfActionModal(invoiceId, clientName, invoiceNum) {
    const filename = `facture_${invoiceNum || invoiceId}.pdf`;

    let existingModal = document.getElementById('pdf-action-modal');
    if (existingModal) {
            ...
            <div class="modal-content shadow border-0">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold">
                        <i class="bi bi-file-earmark-pdf me-2 text-danger"></i>Facture ${invoiceNum ? 'N° ' + invoiceNum : '#' + invoiceId}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
                </div>
                <div class="modal-body text-center py-4">
                    <p class="text-muted mb-4 fs-6">Client : <strong>${clientName || 'Client'}</strong></p>

                    <div id="pdf-modal-loading" class="my-3 py-2">
                        <div class="spinner-border text-primary" role="status"></div>
                        <div class="small text-muted mt-2">Préparation du PDF...</div>
                    </div>

                    <div id="pdf-modal-actions" class="d-none d-grid gap-2">
                        <button type="button" id="btn-modal-share-pdf" class="btn btn-success btn-lg py-3 shadow-sm text-start ps-4">
                            <i class="bi bi-share-fill me-3 fs-5"></i><strong>Envoyer / Partager le PDF</strong>
                            <div class="small text-white-50 ms-4">WhatsApp, E-mail, AirDrop, Messages...</div>
                        </button>
                        <button type="button" id="btn-modal-download-pdf" class="btn btn-outline-primary btn-lg py-3 text-start ps-4">
                            <i class="bi bi-download me-3 fs-5"></i><strong>Télécharger le fichier PDF</strong>
                            <div class="small text-muted ms-4">Enregistrer dans vos fichiers</div>
                        </button>
                    </div>
                </div>
            </div>
    ...
    let pdfBlob = null;
    fetch(`/invoices/${invoiceId}/pdf`)
        .then(res => res.blob())
        .then(blob => {
            pdfBlob = blob;
            document.getElementById('pdf-modal-loading')?.classList.add('d-none');
            document.getElementById('pdf-modal-actions')?.classList.remove('d-none');
        })
        .catch(err => {
            console.error(err);
            document.getElementById('pdf-modal-loading')?.classList.add('d-none');
    ...
    modalEl.querySelector('#btn-modal-share-pdf')?.addEventListener('click', async () => {
        if (!pdfBlob) {
            alert('Le PDF est encore en cours de préparation...');
            return;
        }
    ...
    modalEl.querySelector('#btn-modal-download-pdf')?.addEventListener('click', () => {
        window.location.href = `/invoices/${invoiceId}/pdf`;
        bsModal.hide();
    });
}

function shareOrDownloadPdfById(invoiceId, clientName, invoiceNum) {
    const isMobile = window.innerWidth < 992 || ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);

    if (isMobile && navigator.canShare) {
        showPdfActionModal(invoiceId, clientName, invoiceNum);
    } else {
        window.location.href = `/invoices/${invoiceId}/pdf`;
    }
}
...
async function handleInvoicePdfDownload() {
    ...
    try {
        await shareOrDownloadPdfById(savedId, data.client_name, data.invoice_num);
    } catch (e) {
    ...
    if (wasEditing) {
        showSuccessBanner(`Facture mise à jour. ID : ${savedId}`);
    } else {
